...
test.describe("Search a job and Subscribe for a job", () => {
  ...
  // Scenario 1: Search for a job
  test("Should open the site, navigate to the Job tab, search a job, save it, and check the saved job", async ({
    page,
  }) => {
    await test.step("Step 1: Search and open a job", async () => {
    ...
    await test.step("Step 2: Save and verify the saved job", async () => {
  ...
  // Scenario 2: Subscribe for a job
  test(
    "Should subscribe for a job",
    { tag: "@subscribe" },
    async ({ page }) => {
      searchJobPage = new SearchJobPage(page);

      await test.step("Step 1: Fill in subscription form", async () => {
        await homePage.navigateToJobs();
        await expect(page).toHaveURL(jobsPageURL);
        ...
        await expect(
          searchJobPage.categoryDropdown.locator("option:checked"),
        ).toHaveText(jobCategory);
      ...
      await test.step("Step 2: Sign Up", async () => {
